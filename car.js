...
var render = function()
{
	...
	//camera.lookAt( x1, 0, y1-30 );
	//camera.rotation.z = angle+(3*Math.PI/2) ;

	dist = Math.sqrt(Math.pow((car.position.z - truck.position.z),2)+Math.pow((car.position.x - truck.position.x),2)) ;
...
function move(event){
		...
		if(event.key==='r')
    		{
			car.position.set(-120,0,0);
			truck.position.set(120,0,0);
     		}
